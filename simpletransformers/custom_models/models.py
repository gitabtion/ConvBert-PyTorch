...
from models import ConvBertForPreTraining, ConvBertForMaskedLM
...
class ConvBertForLanguageModelingModel(PreTrainedModel):
    def __init__(self, config, **kwargs):
        super(ConvBertForLanguageModelingModel, self).__init__(config, **kwargs)
        if "generator_config" in kwargs:
            generator_config = kwargs["generator_config"]
        else:
            generator_config = config
        self.generator_model = ConvBertForMaskedLM(generator_config)
        if "discriminator_config" in kwargs:
            discriminator_config = kwargs["discriminator_config"]
        else:
            discriminator_config = config
        self.discriminator_model = ConvBertForPreTraining(discriminator_config)
    ...
    def forward(self, inputs, masked_lm_labels, attention_mask=None, token_type_ids=None):
        ...
        g_out = self.generator_model(
            inputs, labels=masked_lm_labels, attention_mask=attention_mask, token_type_ids=token_type_ids
        )
