...
from models import (
    ...
    ConvBertForMaskedLM, ConvBertForPreTraining
)
...
from simpletransformers.custom_models.models import ElectraForLanguageModelingModel, ConvBertForLanguageModelingModel
...
MODEL_CLASSES = {
    ...
    "convbert": (ConvBertConfig, ConvBertForLanguageModelingModel, ConvBertTokenizer)
}
...
class LanguageModelingModel:
    def __init__(
        ...
        if self.args.tokenizer_name:
            ...
            if self.args.model_name in ['electra', 'convbert']:
        ...
        elif self.args.model_name and self.args.model_name not in ["electra", "convbert"]:
        ...
        if self.args.model_type in ["electra", "convbert"]:
            if generator_name:
                if self.args.model_type == 'electra':
                    self.generator_config = ElectraConfig.from_pretrained(generator_name)
                else:
                    self.generator_config = ConvBertConfig.from_pretrained(generator_name)
            elif self.args.model_name:
                if self.args.model_type == 'electra':
                    ...
                    self.generator_config = ConvBertConfig.from_pretrained(
                        os.path.join(self.args.model_name, "generator_config"), **kwargs,
                    )
            else:
                if self.args.model_type == 'electra':
                    ...
                    self.generator_config = ConvBertConfig(**self.args.generator_config, **kwargs)
                if new_tokenizer:
            ...
            if discriminator_name:
                if self.args.model_type == 'electra':
                    ...
                    self.discriminator_config = ConvBertConfig.from_pretrained(discriminator_name)
            elif self.args.model_name:
                if self.args.model_type == 'electra':
                    ...
                    self.discriminator_config = ConvBertConfig.from_pretrained(
                        os.path.join(self.args.model_name, "discriminator_config"), **kwargs,
                    )
            else:
                if self.args.model_type == 'electra':
                    ...
                    self.discriminator_config = ConvBertConfig(**self.args.discriminator_config, **kwargs)
                if new_tokenizer:
        ...
        if self.args.model_name:
            if self.args.model_type in ["electra", "convbert"]:
                if self.args.model_name in ["electra", "convbert"]:
                    if self.args.model_name == 'electra':
                        ...
                        generator_model = ConvBertForMaskedLM.from_pretrained(generator_name)
                        discriminator_model = ConvBertForPreTraining.from_pretrained(discriminator_name)
                        self.model = ConvBertForLanguageModelingModel(
                            config=self.config,
                            generator_model=generator_model,
                            discriminator_model=discriminator_model,
                            generator_config=self.generator_config,
            ...
            if self.args.model_type in ['electra', 'convbert']:
                if self.args.model_type == "electra":
                    ...
                    generator_model = ConvBertForMaskedLM(config=self.generator_config)
                    discriminator_model = ConvBertForPreTraining(config=self.discriminator_config)
                    self.model = ConvBertForLanguageModelingModel(
                        config=self.config,
                        generator_model=generator_model,
                        discriminator_model=discriminator_model,
    ...
    def train_model(
            self, train_file, output_dir=None, show_running_loss=True, args=None, eval_file=None, verbose=True,
            **kwargs,
    ):
        """
    ...
    def train_tokenizer(self, train_files, tokenizer_name=None, output_dir=None, use_trained_tokenizer=True):
        ...
        if self.args.model_type in ["bert", "electra", "convbert"]:
    ...
    def save_discriminator(self, output_dir=None):
        if self.args.model_type in ["electra", "convbert"]:
            ...
            raise ValueError(
                "Model must be of ElectraForLanguageModelingModel or ConvBertForLanguageModelingModel type")
